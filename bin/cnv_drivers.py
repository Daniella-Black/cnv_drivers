...
apobec= SequenceRange('place_holder', 'place_holder', 38952741, 38992804, 'chr22')
...
if len(cnv.index) > 0:    
    for amp in range(len(cnv.index)):
        cnv_range= SequenceRange('place_holder', 'place_holder', int(cnv['start'][amp]), int(cnv['end'][amp]), cnv[0][amp])
